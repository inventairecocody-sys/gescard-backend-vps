...
class PostgreSQLBackup {
  ...
  async exportWithPgDump() {
    ...
    try {
      ...
      console.log(`✅ Backup créé: ${(stats.size / 1024 / 1024).toFixed(2)} MB en ${Math.round(duration/1000)}s`);
  ...
  async exportManualBackup() {
    ...
    try {
      ...
      console.log(`✅ Backup compressé: ${(stats.size / 1024 / 1024).toFixed(2)} MB (ratio: ${Math.round(stats.size / jsonStats.size * 100)}%)`);
  ...
  async uploadToDrive(filePath, fileName) {
    ...
    try {
      ...
      console.log(`✅ Upload réussi: ${file.data.name} (${sizeMB.toFixed(2)} MB en ${Math.round(duration/1000)}s)`);
  ...
  async getStats() {
    ...
    return {
      ...
      averageSizeMB: this.stats.totalBackups > 0 
